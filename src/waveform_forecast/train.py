...
from waveform_forecast.catalog import (days_since_prev_major, label_hours,
                                       load_aegean_events)
from waveform_forecast.data import MultiStationWindows, fit_stats, stack
from waveform_forecast.evaluate import fold_result, summarise
...
def build_inputs(args):
    ...
    return x, present, labels, dsp, hour_index, feats
...
def run_fold(label, args, x, present, labels, dsp, hour_index, tr_i, va_i, te_i,
    ...
    mk = lambda idx: MultiStationWindows(x, present, labels, args.seq_hours,
                                         idx, stats)
    ...
    return fold_result(label, yt_ref, np.mean(per_seed, axis=0), per_seed,
                       ...
                       model_name=f"multistation[{args.arm}]")


def run(args):
    x, present, labels, dsp, hour_index, feats = build_inputs(args)
    ...
    print(f"  device {device}, seeds {seeds}, arm={args.arm}, embargo {embargo}h")

    results = []
    for name, (tr_i, va_i, te_i) in zip(names, folds):
        r = run_fold(name, args, x, present, labels, dsp, hour_index,
                     tr_i, va_i, te_i, seeds, device, n_feat)
        if r is not None:
            results.append(r)
    if args.cv_folds > 1:
